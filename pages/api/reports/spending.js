...
async function handler(req, res) {
  ...
  // Support parent viewing a child's report via ?child_id=
  const targetId = req.query.child_id || user.id;

  // If parent is requesting a child report, verify ownership
  if (req.query.child_id && user.role === 'parent') {
    const child = await db.user.findFirst({
      where: { id: req.query.child_id, parent_id: user.id },
    });
    if (!child) return res.status(403).json({ error: 'Child not found or not yours' });
  }
  ...
  // Savings progress
  const goals = await db.savingsGoal.findMany({ where: { user_id: targetId } });
